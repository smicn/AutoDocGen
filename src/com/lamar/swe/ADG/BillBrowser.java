...
public class BillBrowser {
	
	private static Customer mCustomer;
	
	public BillBrowser(Customer c) {
		mCustomer = c;
	}
	...
	public static void createDocumentStyles(StyleContext sc) {
		...
		Class baseClass = BillBrowser.class;
	...
	public static void launch() {
		...
		Customer c = BillBrowser.getCustomer();
		if (null == c) {
			DBG.e("BillBrowser: failed to get customer!");
			return;
		}
		JFrame f = new JFrame("AutoDocGen: Bill List of Customer " 
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				...
				if (plain.isSelected()) {
					...
					//
					try {
						os = new FileOutputStream(new File("data/bill.html"));
					} catch (FileNotFoundException e) {
						DBG.e("BillBrowser: failed to write html!");
						e.printStackTrace();
					}
					//
					w = new OutputStreamWriter(os);
					...
					//
					try {
						os = new FileOutputStream(new File("data/bill.doc"));
					} catch (FileNotFoundException e) {
						DBG.e("BillBrowser: failed to write html!");
						e.printStackTrace();
					}
				...
				try {
					DBG.d("w=" + w.toString());
					DBG.d("os=" + os.toString());
					
					DBG.d("pane.doc=" + pane.getDocument().toString());
					int length = pane.getDocument().getLength();
					DBG.d("pane.doc.length=" + pane.getDocument().getLength());
					DBG.d("pane.doc.text: " + pane.getDocument().getText(0, length));
